...
def coupon_generation(effect, term, freq, rule, fixing_days, payment_days,
    ...
    for i in range(coupon_length):
        fixing_date  = cal.adjust(schedule[i]  + ql.Period(-fixing_days, ql.Days), fixing_conv) 
        payment_date = cal.adjust(schedule[i+1]+ ql.Period(payment_days, ql.Days), payment_conv) 
